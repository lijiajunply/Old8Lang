...
public class AnyLangValue : LangValueType
{
    ...
    // ===== V1 兼容接口 =====
    // 为了减少迁移工作量，提供 V1 的接口别名

    /// <summary>
    /// V1 兼容：类标识符（现在指向 ClassId）
    /// </summary>
    public LangId Id => ClassId;

    /// <summary>
    /// V1 兼容：实例作用域管理器（现在指向 InstanceScope）
    /// </summary>
    public VariateManager Manager => InstanceScope;

    /// <summary>
    /// V1 兼容：实例字段字典（模拟 V1 的 Result）
    /// 注意：V1 使用 ClassMemberId 作为 key，V2 使用 string
    /// </summary>
    public Dictionary<ClassMemberId, LangExpression> Result =>
        InstanceData.ToDictionary(
            kv => new ClassMemberId(kv.Key),
            kv => (LangExpression)kv.Value
        );

    /// <summary>
    /// V1 兼容：外部管理器（现在指向 InstanceScope）
    /// </summary>
    public VariateManager ExternalManager
    {
        get => InstanceScope;
        set
        {
            // V2 不支持直接设置 ExternalManager
            // 如果需要设置，应该操作 InstanceScope 的内容
            // 这里提供空实现以兼容 V1 代码
        }
    }

    /// <summary>
    /// V1 兼容：清除函数查找缓存（V2 不需要此功能）
    /// </summary>
    public void ClearFunctionLookupCache()
    {
        // V2 使用 MethodTable，不需要缓存清理
    }

    /// <summary>
    /// V1 兼容：设置变量（映射到 V2 的 SetField）
    /// </summary>
    public void Set(LangId id, LangValueType value)
    {
        // 如果是字段，更新 InstanceData
        if (InstanceData.ContainsKey(id.IdName))
        {
            InstanceData[id.IdName] = value;
        }

        // 同时更新 InstanceScope
        InstanceScope.Set(id, value);
    }

    /// <summary>
    /// V1 兼容：变量字典
    /// </summary>
    public Dictionary<ClassMemberId, LangExpression> Variates =>
        InstanceData.ToDictionary(
            kv => new ClassMemberId(kv.Key),
            kv => (LangExpression)kv.Value
        );
    ...
    /// V1 兼容：获取父类实例
    /// </summary>
    public LangValueType? GetSuperInstance()
    {
        // TODO: 实现 super 支持
        return null;
    }
