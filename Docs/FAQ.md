...
**A**: 因为我发现很多人真会认为 = 是 等于而不是赋值，且有些人会认为是把左值赋值给右值。现在看来很难以置信，但是在我设计这门语言那会儿，真有人这么以为并且问我问题。这件事让我意识到：还是把赋值设计成一个更 “直观” 的符合吧！
```old8
a <- 10        // 变量声明和赋值
a:int <- 10    // 带类型标注的赋值
b:const <- 10  // 设置一个常量，因为常量是不变的，所以常量的类型假注也是恒定的，为右值结果
```
...
- **列表**: 使用花括号 `{1, 2, 3}` (动态大小)
- **数组**: 使用方括号 `[1, 2, 3]` (固定大小)
- **字典**: 使用花括号带冒号 `{"key": value}`。可以使用 dict() 来新建一个字典。

```old8
list <- {1, 2, 3}             // 列表
array <- [1, 2, 3]            // 数组
dict <- {"name": "Old8"}      // 字典
dict_new <- dict()            // 新建一个空字典
```
...
**A**: **支持**。Old8Lang 支持继承，同时支持泛型，Minix，多态。
...
# 虚拟机模式
dotnet run --project Old8Lang.App -- -vm mycode.old8
```
