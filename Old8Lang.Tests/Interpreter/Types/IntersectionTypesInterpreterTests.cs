...
result <- processItem(item)";
        ...
        Assert.Equal(0, ((IntLangValue)result).Value); // 克隆的对象相等
    }
...
resultA <- obj.methodA()";
        ...
        // Assert
        var resultA = interpreter.Manager.GetValue(new LangId("resultA"));
        Assert.IsType<StringLangValue>(resultA);
        Assert.Equal("Method A", ((StringLangValue)resultA).Value);
    }
...
result2 <- processor2.process()";
        ...
        Assert.Equal("Processed: Hello", ((StringLangValue)result1).Value);

        Assert.IsType<StringLangValue>(result2);
        Assert.Equal("Processed: 42", ((StringLangValue)result2).Value);
    }
