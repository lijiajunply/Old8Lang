...
# 虚拟机测试脚本
echo "Running virtual machine tests..."
...
# 获取所有测试文件
test_files=$(find ./VirtualMachine -name "*.old8")
...
for file in $test_files; do
    ...
    # 运行测试，添加10秒超时限制
    # 使用兼容bash 3.2的方式实现超时
    dotnet run --project Old8Lang.App -- -vm "$file" &
    DOTNET_PID=$!
...
echo ""
echo "------------------------------"
echo "=== Virtual Machine Test Results ==="
...
if [ $failed -eq 0 ]; then
    echo "🎉 All virtual machine tests passed!"
    exit 0
else
    echo "❌ Some virtual machine tests failed!"
    exit 1
fi
