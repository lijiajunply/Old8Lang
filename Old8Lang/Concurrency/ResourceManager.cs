...
public static class ResourceManager
{
    ...
    /// <param name="storage">资源存储字典</param>
    /// <param name="resourceTypeName">资源类型名称（用于异常消息）</param>
    /// <returns>资源包装器</returns>
    private static ResourceWrapper<T> ValidateAndGetResource<T>(
        int resourceId,
        ConcurrentDictionary<int, ResourceWrapper<T>> storage,
        string resourceTypeName)
        where T : class
    {
        if (!storage.TryGetValue(resourceId, out var wrapper))
        {
    ...
    {
        var wrapper = ValidateAndGetResource(semaphoreId, Semaphores, "Semaphore");
        wrapper.UpdateLastAccessTime();
        wrapper.Resource.Wait();
    }
    ...
    {
        var wrapper = ValidateAndGetResource(atomicId, AtomicInts, "AtomicInt");
        wrapper.UpdateLastAccessTime();
    ...
    {
        var wrapper = ValidateAndGetResource(latchId, CountDownLatches, "倒计时锁");
    ...
    {
        var wrapper = ValidateAndGetResource(barrierId, CyclicBarriers, "循环栅栏");
    ...
    {
        var wrapper = ValidateAndGetResource(ctsId, CancellationTokenSources, "取消令牌源");
        wrapper.UpdateLastAccessTime();
