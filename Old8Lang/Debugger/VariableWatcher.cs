...
{
    private readonly List<WatchedVariable> _watchedVariables = new();
    ...
    {
        var watch = _watchedVariables.FirstOrDefault(w => w.Expression == expression);
        if (watch != null)
        {
            return _watchedVariables.Remove(watch);
        }
        return false;
    }
    ...
    {
        return _watchedVariables.ToList();
    }
    ...
    {
        foreach (var watch in _watchedVariables.Where(w => w.IsEnabled))
        {
            EvaluateWatch(watch, manager);
        }
    }
...
public class StackFrame
{
    ...
    public override string ToString()
    {
        return $"{FunctionName} ({System.IO.Path.GetFileName(FilePath)}:{Line})";
    }
}
...
public class CallStack
{
    ...
    {
        return _frames.Count > 0 ? _frames.Pop() : null;
    }
