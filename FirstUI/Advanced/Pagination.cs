...
public class Pagination : WidgetBase
{
    ...
    public override object Build(BuildContext context)
    {
        ...
        if (ShowTotalInfo && TotalItems > 0)
        {
            var infoText = new TextBlock
            {
                ...
                Foreground = Utils.LayoutHelper.ParseColorBrush("#666666")
            };
    ...
    private Control CreatePageButton(string text, int targetPage, bool disabled, bool isActive = false)
    {
        ...
        if (isActive)
        {
            button.Background = Utils.LayoutHelper.ParseColorBrush("#2196F3");
            button.Foreground = Utils.LayoutHelper.ParseColorBrush("#FFFFFF");
            button.BorderThickness = new Avalonia.Thickness(0);
        }
        else
        {
            button.Background = Utils.LayoutHelper.ParseColorBrush("#F5F5F5");
            button.Foreground = Utils.LayoutHelper.ParseColorBrush("#333333");
