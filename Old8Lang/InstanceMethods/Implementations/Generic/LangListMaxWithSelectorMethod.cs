...
/// <summary>
/// ILangList.Max(selector) - 对列表元素应用选择器后求最大值
/// </summary>
public class LangListMaxWithSelectorMethod : BaseLangListMethod
{
    ...
    /// 方法文档
    /// </summary>
    public override string? Documentation => "对列表元素应用选择器后求最大值";
        ...
        VariateManager manager, SourcePosition position)
    {
        ...
        if (items.Count == 0)
        {
            throw new InvalidOperationError(position, "无法对空列表求最大值");
        }
        ...
        var maxValue = selector.Run(manager, [items[0]]);

        for (int i = 1; i < items.Count; i++)
        {
            var currentValue = selector.Run(manager, [items[i]]);

            if (CompareValues(currentValue, maxValue) > 0)
            {
    ...
    {
        throw new NotSupportedException("Max(selector) 方法暂不支持编译模式");
    }
    ...
    protected override object? ExecuteInVMInternal(object? instance, object?[] arguments)
    {
        ...
        var maxValue = vm.CallFunctionObject(selector, [items[0]]);

        for (int i = 1; i < items.Count; i++)
        {
            var currentValue = vm.CallFunctionObject(selector, [items[i]]);

            if (CompareValuesVM(currentValue, maxValue) > 0)
            {
                maxValue = currentValue;
            }
        }
